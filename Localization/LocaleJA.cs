    ...
    public sealed class LocaleJA : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.BusBoardingSpeedFactor)), "<1x = バニラ>\n値を上げるとバス停での乗車/積み込み時間が短くなります。\n通常の待ち行列は早く処理されますが、バニラ設計上、遅れた乗客がまだ発車を遅らせることがあります。\n単独の遅れたシムに車両を逃させたい場合は [✓] <遅れたシムを待たずに車両を発車> を使ってください。\n2x はおよそ2倍の乗車速度です。" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.RailBoardingSpeedFactor)), "鉄道乗車速度" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.RailBoardingSpeedFactor)), "<1x = バニラ>\n列車、路面電車、地下鉄の停留所に適用されます。\n値を上げると乗車/積み込み時間が短くなります。\n単独の遅れたシムがまだ詰まらせる場合は [✓] <遅れたシムを待たずに車両を発車> を使ってください。" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.WaterBoardingSpeedFactor)), "船 + フェリー速度" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.WaterBoardingSpeedFactor)), "<1x = バニラ>\n船とフェリーの停留所に適用されます。\n値を上げると乗車/積み込み時間が短くなります。\n単独の遅れたシムがまだ詰まらせる場合は [✓] <遅れたシムを待たずに車両を発車> を使ってください。" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.AirBoardingSpeedFactor)), "飛行機搭乗速度" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.AirBoardingSpeedFactor)), "<1x = バニラ>\n旅客機ターミナルに適用されます。\n値を上げると搭乗/積み込み時間が短くなります。\n単独の遅れたシムがまだ詰まらせる場合は [✓] <遅れたシムを待たずに車両を発車> を使ってください。" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.CancelLateBoarders)), "遅れたシムを待たずに車両を発車" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.CancelLateBoarders)), "**実験的BETA**\nバニラの発車時刻後も<準備未完了>の単独市民は、その車両を逃すことができます。\n注意: 一緒に移動するグループ/家族はまだ<スキップされません>。バニラと同じように遅延を起こす場合があります。\nスキップされた市民は削除されません。その後はバニラのシステムが処理を続けます。" },

                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.StatusOverview)), "総利用数" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.StatusOverview)), "ゲームの交通情報ビューから取得した月間公共交通利用数。\n更新時刻はこのステータスのスナップショット時刻です。" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.StatsToLog)), "**FastBoarding.log** に1回だけ詳細レポートを書き込みます。\n待機数、各交通モードの最悪停留所トップ3、スキップされたシム例、エンティティID、路線ヒントを含みます。" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.OpenLog)), "ログを開く" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.OpenLog)), "**FastBoarding.log** があれば開きます。\nファイルがまだない場合は Logs フォルダーを開きます。" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.EnableVerboseLogging)), "**デバッグ / テスト専用**\n都市実行中に <FastBoarding.log> へライブ診断を追加します。\n**通常プレイでは有効にしないでください。**\nオンのままだとパフォーマンスが低下し、巨大なログファイルが作られる可能性があります。" }
            };
        }
